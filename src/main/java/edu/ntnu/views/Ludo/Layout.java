...
/**
 * Layout is responsible for rendering the visual layout of the Ludo game board,including player
 * zones, paths, and name labels.
 */
public class Layout {

  int x, y, width, height;
  ArrayList<edu.ntnu.model.Player> logicPlayers;
  //The logical players whose names and colors are displayed
  ...
  /**
   * Draws the Ludo game board including player start zones, paths, and the player names at
  ...
  public void draw(GraphicsContext gc) {
    ...
    for (int i = 0; i < 2; i++) {
      for (int j = 0; j < 2; j++) {
        ...
        gc.setFill(COLOR_3);
        gc.fillRect(temp1 + (2 * i * width) + 9 * width, temp2 + (2 * j * height) + 9 * height,
            width, height);
        gc.setFill(COLOR_1);
    ...
    for (int i = 0; i < 2; i++) {
      for (int j = 0; j < 2; j++) {
        ...
        gc.strokeRect(temp1 + (2 * i * width) + 9 * width, temp2 + (2 * j * height) + 9 * height,
            width, height);
        gc.strokeRect(temp1 + (2 * i * width) + 9 * width, temp2 + (2 * j * height), width, height);
        gc.strokeRect(temp1 + (2 * i * width), temp2 + (2 * j * height) + 9 * height, width,
            height);
      }
    }

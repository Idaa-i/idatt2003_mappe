...
public class GameMoves {
  ...
  public void handleMouseClick(MouseEvent e) {
    if (flag != 1 || p.players[currentPlayer] == null) {
      return;
    }
  ...
  private int checkForPlayableMove() {
    ...
    if (dice == 6) {
      for (int i = 0; i < 4; i++) {
        if (p.players[currentPlayer].pawns[i].current == -1) {
          return 1;
        }
      }
    }
  ...
  private void handleCollision(int movedPawnIndex) {
    ...
    if ((pos % 13) == 0 || (pos % 13) == 8 || pos >= 51) {
      return;
    }
    for (int i = 0; i < p.players.length; i++) {
      if (i == currentPlayer || p.players[i] == null) {
        continue;
      }
      for (int j = 0; j < 4; j++) {
